package waveletmatrix

import (
	"bytes"
	"testing"
)

// naiveRank counts occurrences of c in seq[0..i) by brute force.
func naiveRank(seq []byte, c byte, i int) int {
	count := 0
	for _, b := range seq[:i] {
		if b == c {
			count++
		}
	}
	return count
}

func TestRankEmpty(t *testing.T) {
	m := Build(nil)
	for c := 0; c < 256; c++ {
		if got := m.Rank(byte(c), 0); got != 0 {
			t.Errorf("Rank(%d, 0) on empty = %d, want 0", c, got)
		}
		if got := m.Rank(byte(c), 5); got != 0 {
			t.Errorf("Rank(%d, 5) on empty = %d, want 0", c, got)
		}
	}
}

func TestRankSingleChar(t *testing.T) {
	seq := []byte{42}
	m := Build(seq)

	if got := m.Rank(42, 1); got != 1 {
		t.Errorf("Rank(42, 1) = %d, want 1", got)
	}
	if got := m.Rank(42, 0); got != 0 {
		t.Errorf("Rank(42, 0) = %d, want 0", got)
	}
	if got := m.Rank(43, 1); got != 0 {
		t.Errorf("Rank(43, 1) = %d, want 0", got)
	}
}

func TestRankAllSameChar(t *testing.T) {
	seq := bytes.Repeat([]byte{97}, 10) // "aaaaaaaaaa"
	m := Build(seq)
	for i := 0; i <= 10; i++ {
		if got := m.Rank(97, i); got != i {
			t.Errorf("Rank('a', %d) = %d, want %d", i, got, i)
		}
		if got := m.Rank(98, i); got != 0 {
			t.Errorf("Rank('b', %d) = %d, want 0", i, got)
		}
	}
}

func TestRankMatchesNaive(t *testing.T) {
	cases := []struct {
		name string
		seq  []byte
	}{
		{"banana", []byte("banana")},
		{"mississippi", []byte("mississippi")},
		{"abracadabra", []byte("abracadabra")},
		{"the quick brown fox", []byte("the quick brown fox")},
		{"all_bytes", func() []byte {
			b := make([]byte, 256)
			for i := range b {
				b[i] = byte(i)
			}
			return b
		}()},
		{"repeated_pattern", bytes.Repeat([]byte("abc"), 20)},
	}

	for _, tc := range cases {
		tc := tc
		t.Run(tc.name, func(t *testing.T) {
			m := Build(tc.seq)
			n := len(tc.seq)

			testChars := []byte{'a', 'b', 'c', 'm', 'i', 's', 0, 255, 128, 'n'}
			testPos := []int{0, 1, n / 4, n / 2, 3 * n / 4, n}

			for _, c := range testChars {
				for _, pos := range testPos {
					if pos < 0 || pos > n {
						continue
					}
					got := m.Rank(c, pos)
					want := naiveRank(tc.seq, c, pos)
					if got != want {
						t.Errorf("seq=%q Rank(%d, %d) = %d, want %d", tc.name, c, pos, got, want)
					}
				}
			}
		})
	}
}

func TestRankBoundaryClamp(t *testing.T) {
	seq := []byte("hello")
	m := Build(seq)

	want := naiveRank(seq, 'l', len(seq))
	if got := m.Rank('l', 1000); got != want {
		t.Errorf("Rank('l', 1000) = %d, want %d", got, want)
	}
	for _, c := range []byte("hello") {
		if got := m.Rank(c, 0); got != 0 {
			t.Errorf("Rank(%c, 0) = %d, want 0", c, got)
		}
	}
}

func TestRankSentinelByte(t *testing.T) {
	seq := []byte{'a', 0, 'b', 0, 'c'}
	m := Build(seq)
	for i := 0; i <= len(seq); i++ {
		got := m.Rank(0, i)
		want := naiveRank(seq, 0, i)
		if got != want {
			t.Errorf("Rank(0, %d) = %d, want %d", i, got, want)
		}
	}
}

func TestRankFullByteRange(t *testing.T) {
	seq := make([]byte, 256)
	for i := range seq {
		seq[i] = byte(i)
	}
	m := Build(seq)
	for c := 0; c < 256; c++ {
		if got := m.Rank(byte(c), c); got != 0 {
			t.Errorf("Rank(%d, %d) = %d, want 0", c, c, got)
		}
		if got := m.Rank(byte(c), c+1); got != 1 {
			t.Errorf("Rank(%d, %d) = %d, want 1", c, c+1, got)
		}
	}
}

func TestPersistence(t *testing.T) {
	seq := []byte("abracadabra")
	m := Build(seq)

	var buf bytes.Buffer
	if _, err := m.WriteTo(&buf); err != nil {
		t.Fatalf("WriteTo failed: %v", err)
	}

	m2, err := ReadFrom(&buf)
	if err != nil {
		t.Fatalf("ReadFrom failed: %v", err)
	}

	for _, c := range []byte("abcdr") {
		for i := 0; i <= len(seq); i++ {
			got := m2.Rank(c, i)
			want := m.Rank(c, i)
			if got != want {
				t.Errorf("after round-trip: Rank(%c, %d) = %d, want %d", c, i, got, want)
			}
		}
	}
}

func TestPersistenceEmpty(t *testing.T) {
	m := Build(nil)
	var buf bytes.Buffer
	if _, err := m.WriteTo(&buf); err != nil {
		t.Fatalf("WriteTo failed: %v", err)
	}
	m2, err := ReadFrom(&buf)
	if err != nil {
		t.Fatalf("ReadFrom failed: %v", err)
	}
	if got := m2.Rank('a', 5); got != 0 {
		t.Errorf("Rank on empty after round-trip = %d, want 0", got)
	}
}

func TestMatchesNaiveExhaustive(t *testing.T) {
	seq := []byte("mississippi")
	m := Build(seq)
	n := len(seq)
	for c := 0; c < 256; c++ {
		for i := 0; i <= n; i++ {
			got := m.Rank(byte(c), i)
			want := naiveRank(seq, byte(c), i)
			if got != want {
				t.Errorf("Rank(%d, %d) = %d, want %d", c, i, got, want)
			}
		}
	}
}
