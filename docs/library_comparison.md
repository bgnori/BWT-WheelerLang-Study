...
- **textindex**（本リポジトリ） — `github.com/bgnori/textindex`
...
| 項目 | textindex | shenwei356/bwt | rossmerr/fm-index |
|---|---|---|---|
...
| 構造 | textindex | shenwei356/bwt/fmi | rossmerr/fm-index |
|---|---|---|---|
...
| 機能 | textindex | shenwei356/bwt/fmi | rossmerr/fm-index |
|---|---|---|---|
...
| | textindex | shenwei356/bwt/fmi | rossmerr/fm-index |
|---|---|---|---|
...
| 制限 | textindex | shenwei356/bwt/fmi | rossmerr/fm-index |
|---|---|---|---|
...
| 観点 | 推奨ライブラリ | 理由 |
|---|---|---|
| **日本語・マルチバイト文字の検索** | textindex | 8bitフル対応、UTF-8を正式サポート |
| **近似検索（タイポ・変異許容）** | shenwei356/bwt/fmi | k-mismatch（Hamming距離）をサポートする唯一の実装 |
| **インデックス保存・配布** | textindex | 唯一の永続化機能 |
| **正規表現検索** | textindex | 星なし正規表現（?\|\|{n,m}）をサポート |
| **大規模テキストの省メモリ** | rossmerr/fm-index（概念上） | サンプリングSA+Wavelet Treeだが、Rank O(n)で実用速度に課題 |
| **ゼロ依存・シンプル導入** | shenwei356/bwt | 外部依存なし、`go get`一発 |
| **学習・Wheeler グラフ理解** | textindex | Mermaid出力・TUIブラウザ・Wheeler グラフとの対応が明示的 |

---

## 9. 設計思想の違い

- **textindex**：Wheeler グラフとしてのFM-indexを教育・研究目的で実装。8bitフル対応・永続化・星なし正規表現・2種のSAアルゴリズムを備えた最も機能豊富な実装。
